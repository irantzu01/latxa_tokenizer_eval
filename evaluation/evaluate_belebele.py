#!/usr/bin/env python3
"""
Evaluate models on Belebele dataset (Basque reading comprehension).
Choose which model to evaluate via command line argument.
"""
...
def format_question(item):
    """Format a Belebele question for the model."""
...
def build_fewshot_example(item):
    ...
    answer_idx = ['1', '2', '3', '4'].index(item["correct_answer_num"])
    return format_question(item) + " " + answer2letter[answer_idx]
...
def evaluate_static_model(model, tokenizer, dataset, shots=5, device='cuda'):
    ...
    for idx, item in enumerate(tqdm(dataset, desc="Evaluating")):
        ...
        gold = ['1', '2', '3', '4'].index(item["correct_answer_num"])
...
def main():
    ...
    results_file = f"results/belebele_{args.model}_{args.shots}shot.jsonl"
